...
package com.hmx.ide.activities.editor
...
import com.hmx.ide.databinding.LayoutSearchProjectBinding
import com.hmx.ide.flashbar.Flashbar
import com.hmx.ide.fragments.sheets.ProgressSheet
import com.hmx.ide.handlers.LspHandler.connectClient
import com.hmx.ide.handlers.LspHandler.destroyLanguageServers
import com.hmx.ide.lookup.Lookup
import com.hmx.ide.lsp.IDELanguageClientImpl
import com.hmx.ide.lsp.java.utils.CancelChecker
import com.hmx.ide.preferences.internal.GeneralPreferences
import com.hmx.ide.projects.GradleProject
import com.hmx.ide.projects.builder.BuildService
import com.hmx.ide.projects.internal.ProjectManagerImpl
import com.hmx.ide.services.builder.GradleBuildService
import com.hmx.ide.services.builder.GradleBuildServiceConnnection
import com.hmx.ide.services.builder.gradleDistributionParams
import com.hmx.ide.tasks.executeAsyncProvideError
import com.hmx.ide.tasks.executeWithProgress
import com.hmx.ide.tooling.api.messages.AndroidInitializationParams
...
import com.hmx.ide.tooling.api.messages.result.InitializeResult.Failure.PROJECT_DIRECTORY_INACCESSIBLE
import com.hmx.ide.tooling.api.messages.result.InitializeResult.Failure.PROJECT_NOT_DIRECTORY
import com.hmx.ide.tooling.api.messages.result.InitializeResult.Failure.PROJECT_NOT_FOUND
import com.hmx.ide.tooling.api.models.BuildVariantInfo
import com.hmx.ide.tooling.api.models.mapToSelectedVariants
import com.hmx.ide.utils.DURATION_INDEFINITE
import com.hmx.ide.utils.DialogUtils.newMaterialDialogBuilder
import com.hmx.ide.utils.RecursiveFileSearcher
import com.hmx.ide.utils.flashError
import com.hmx.ide.utils.flashbarBuilder
import com.hmx.ide.utils.resolveAttr
import com.hmx.ide.utils.showOnUiThread
import com.hmx.ide.utils.withIcon
import com.hmx.ide.viewmodel.BuildVariantsViewModel
...
val findInProjectDialog: AlertDialog
  ...
  private fun notifySyncNeeded(onConfirm: () -> Unit) {
    ...
    if (buildService == null || editorViewModel.isInitializing) return
  ...
  fun initializeProject(buildVariantsProvider: () -> Map<String, String>) {
    executeWithProgress { progress ->
      executeAsyncProvideError(buildVariantsProvider::invoke) { result, error ->
        com.hmx.ide.tasks.runOnUiThread {
          progress.dismiss()
        }
  ...
  protected open fun postProjectInit(isSuccessful: Boolean, failure: InitializeResult.Failure?) {
    val manager = ProjectManagerImpl.getInstance()
    if (!isSuccessful) {
