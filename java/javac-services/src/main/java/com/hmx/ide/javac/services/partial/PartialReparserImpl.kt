...
package com.hmx.ide.javac.services.partial

import com.hmx.ide.javac.services.compiler.JavacFlowListener
import com.hmx.ide.javac.services.util.ReparserUtils
import com.hmx.ide.javac.services.visitors.FindAnonymousVisitor
import com.hmx.ide.javac.services.visitors.TranslateMethodPositionsVisitor
...
class PartialReparserImpl : PartialReparser {
  ...
  override fun reparseMethod(
    ...
    try {
      val l = com.hmx.ide.javac.services.NBLog.instance(context)
  ...
  private fun newParser(
    ...
    val factory =
      com.hmx.ide.javac.services.NBParserFactory.instance(context)
        as com.hmx.ide.javac.services.NBParserFactory
    val scannerFactory = ScannerFactory.instance(context)
    val cancelService = com.hmx.ide.javac.services.CancelService.instance(context)
    val lexer = scannerFactory.newScanner(buf, true)
    if (
      endPositions
        is com.hmx.ide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
    ) {
      endPositions.resetErrorEndPos()
    }

    return object :
      com.hmx.ide.javac.services.NBParserFactory.NBJavacParser(
        factory,
        lexer,
        true,
        false,
